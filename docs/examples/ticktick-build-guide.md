...
3. [Natural Language Parser](#3-natural-language-parser)
...
10. [Full Build Roadmap](#10-full-build-roadmap)

---

## 1. Data Model & Schema

### Core Task Object (TypeScript)
...
The core challenge: users expect instant UI with reliable sync across devices. The solution is a 3-layer architecture:
...
```
Input:  "Call dentist tomorrow at 3pm high priority #health every week"
Output: {
  title: "Call dentist",
  dueDate: "2024-03-16",
  dueTime: "15:00",
...
export function parseTaskInput(input: string): ParsedTask {
  ...
  const recurrencePatterns = [
    ...
    { re: /\bevery (\d+) days?\b/i,          rrule: (m) => `FREQ=DAILY;INTERVAL=${m[1]}` },
    { re: /\bevery (mon|tue|wed|thu|fri|sat|sun)/i, rrule: (m) => {
...
Build these views in order of priority:
...
```typescript
// components/SwipeableTask.tsx
import { useSpring, animated } from '@react-spring/web';
import { useDrag } from '@use-gesture/react';

function SwipeableTask({ task, onComplete, onDelete }) {
...
### Supabase Setup (Recommended Backend)
...
// Row Level Security policies (run in Supabase SQL editor)
/*
...
// 1. Calculate next reminder time
...
// 4. Server-side cron job (runs every minute)
...
```typescript
// gamification/scoring.ts

const POINT_VALUES = {
  COMPLETE_TASK:        10,
  COMPLETE_HIGH_PRIORITY: 25,
  COMPLETE_ON_TIME:     15,   // Completed before due date/time
  COMPLETE_EARLY:       20,   // Completed 1+ day early
  ...
  COMPLETE_ALL_TODAY:   30,   // Cleared all of today's tasks
  CREATE_TASK:           2,
};
...
function getStreakStatus(stats: UserStats) {
  ...
  const atRisk   = stats.lastActiveDate === yesterday;
...
function buildHeatmapData(completionHistory: string[], weeks = 52) {
...
> Never render all tasks at once. Virtualize everything. Filter at the selector level, not the render level.
...
// Only recomputes when tasks or filter actually change
const selectFilteredTasks = createSelector(
  ...
  (tasks, filter) => {
    ...
    if (filter.priority != null)   result = result.filter(t => t.priority === filter.priority);
...
-- Good: uses indexes, filtered, paginated
SELECT * FROM tasks
WHERE created_by = $1
...
WHERE created_by = $1
  AND deleted_at IS NULL
  AND ($2::uuid IS NULL OR id > $2)   -- cursor
...
## 10. Full Build Roadmap

### Phase 1: Core MVP (Weeks 1–4)

| Week | Focus |
|------|-------|
| 1 | Project setup, Supabase Auth, basic task CRUD, local Zustand state |
| 2 | Lists, tags, priorities, due dates — full data model implemented |
| 3 | Today view, Inbox, All Tasks — core navigation shell |
| 4 | Offline support, sync engine v1, IndexedDB/SQLite local store |
...
| Week | Focus |
|------|-------|
| 5 | Natural language parser, quick-add bar |
| 6 | Recurring tasks (rrule), subtasks |
| 7 | Push notifications, reminders |
| 8 | Drag & drop reorder, swipe actions (mobile) |

**Ship:** Feature-complete for power users.

---

### Phase 3: Views & Productivity (Weeks 9–12)

| Week | Focus |
|------|-------|
| 9  | Calendar view (react-big-calendar) |
| 10 | Kanban view (columns by list or status) |
| 11 | Filters, full-text search, bulk actions |
| 12 | Keyboard shortcuts, command palette (cmdk) |
...
| Week | Focus |
|------|-------|
| 13 | Shared lists, member invites, real-time multi-user updates |
| 14 | Gamification — points, streaks, habits, contribution heatmap |
| 15 | File attachments, rich notes (markdown editor) |
| 16 | Performance audit — virtualization, query optimization, bundle size |
...
*Generated with Claude · ~800 lines of production code examples across 10 sections*
