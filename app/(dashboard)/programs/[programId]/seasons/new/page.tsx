...
export default async function NewSeasonPage({
  ...
  if (!scope.databaseReady) {
    return (
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold tracking-tight">New season</h2>
        <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 dark:border-amber-700 dark:bg-amber-950/40">
          <p className="text-sm text-amber-900 dark:text-amber-200">
            {scope.errorMessage ?? "Unable to load season creation right now."}
          </p>
        </div>
      </section>
    );
  ...
  const startDate = readSearchParam(resolvedSearchParams, "startDate");
  const endDate = readSearchParam(resolvedSearchParams, "endDate");
  ...
  return (
    <section className="space-y-4">
      ...
      <form
        action={`/programs/${program.id}/seasons/create`}
        method="post"
