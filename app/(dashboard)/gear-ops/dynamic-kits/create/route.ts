...
export async function POST(request: Request) {
  ...
  const access = await resolveGearOpsReadAccess({
    ...
    workflow: "gear-ops.dynamic-kits.create",
  });
  ...
  if (!name) {
    redirect("/gear-ops/dynamic-kits/new");
  }

  const def = await db.dynamicKitDefinition.create({
    data: {
      organizationId,
      name,
      description,
  ...
  redirect(`/gear-ops/dynamic-kits/${def.id}`);
}
