...
export default async function EditConsumableTransactionPage({
  params,
  searchParams,
}: {
  params: Promise<{ itemId: string; transactionId: string }>;
  ...
  if (!scope.databaseReady) {
    return (
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold tracking-tight">Edit consumable transaction</h2>
        <ErrorMessage message={scope.errorMessage ?? "Unable to load consumable transaction edit right now."} />
      </section>
    );
  }
  ...
  const access = await resolveGearOpsReadAccess({
    ...
    workflow: "gear-ops.items.consumables.edit.access",
  });
  ...
  let queryErrorMessage = "Unable to load consumable transaction edit right now. Please try again later.";

  try {
    [item, transaction, events] = await Promise.all([
      db.gearItem.findFirst({
        where: { id: itemId, AND: [access.where] },
        select: { id: true, name: true, inventoryType: true },
      }),
      db.consumableTransaction.findFirst({
        where: { id: transactionId, gearItemId: itemId, organizationId: scope.organizationId },
        select: {
          id: true,
  ...
  if (!transaction) {
    return (
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold tracking-tight">Edit consumable transaction</h2>
        <div className="rounded-lg border bg-white p-4 dark:bg-zinc-900">
          <p className="text-sm text-zinc-600 dark:text-zinc-400">Consumable transaction not found for this gear item.</p>
        </div>
        <BackLink href={`/gear-ops/items/${item.id}`} label={item.name} />
      </section>
    );
  ...
  const transactionType =
    readSearchParam(resolvedSearchParams, "transactionType") || transaction.transactionType;
  const quantityDelta = readSearchParam(resolvedSearchParams, "quantityDelta") || String(transaction.quantityDelta);
  const recordedAt =
    readSearchParam(resolvedSearchParams, "recordedAt") || formatDateTimeInputValue(transaction.recordedAt);
  const eventId = readSearchParam(resolvedSearchParams, "eventId") || transaction.eventId || "";
  const notes = readSearchParam(resolvedSearchParams, "notes") || transaction.notes || "";
    ...
    <section className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          ...
          {transaction.quantityDelta}) · Recorded {formatGearOpsDateTime(transaction.recordedAt)} · Logged{" "}
          {formatGearOpsDateTime(transaction.createdAt)} by {transaction.recordedBy.firstName} {transaction.recordedBy.lastName}
        </p>
      </div>
      ...
      <form
        action={`/gear-ops/items/${item.id}/consumables/${transaction.id}/edit/update`}
        method="post"
      ...
      >
        ...
        <FormActions submitLabel="Save transaction" cancelHref={`/gear-ops/items/${item.id}`} />
      </form>
    </section>
  );
}
