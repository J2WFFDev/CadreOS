...
export default async function EditGearAssignmentPage({
  params,
  searchParams,
}: {
  params: Promise<{ itemId: string; assignmentId: string }>;
  ...
  if (!scope.databaseReady) {
    return (
      <section className="space-y-4">
        <h2 className="text-2xl font-semibold tracking-tight">Edit assignment</h2>
        <ErrorMessage message={scope.errorMessage ?? "Unable to load assignment edit right now."} />
      </section>
    );
  }
  ...
  const access = await resolveGearOpsReadAccess({
    ...
    workflow: "gear-ops.assignments.edit.access",
  });
  ...
  let queryErrorMessage = "Unable to load assignment edit right now. Please try again later.";

  try {
    [item, assignment, people, teams, events] = await Promise.all([
      db.gearItem.findFirst({
        where: { id: itemId, AND: [access.where] },
        select: { id: true, name: true },
      }),
      db.gearAssignment.findFirst({
        where: { id: assignmentId, gearItemId: itemId, organizationId: scope.organizationId },
        select: {
          id: true,
          status: true,
    ...
    if (isSchemaUnavailableError(error)) {
      queryErrorMessage = "Database schema is not available yet. Run database setup before editing assignments.";
    }
  }
  ...
  if (!assignment) {
    return (
      <section className="space-y-4">
        ...
        <div className="rounded-lg border bg-white p-4 dark:bg-zinc-900">
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            Assignment not found for this gear item.
          </p>
        </div>
        <BackLink href={`/gear-ops/items/${itemId}`} label={item.name} />
      </section>
    );
  }
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="space-y-1">
        <h2 className="text-2xl font-semibold tracking-tight">Edit assignment</h2>
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          Item: {item.name} · Assigned {formatGearOpsDateTime(assignment.assignedAt)} by{" "}
      ...
      <form
        action={`/gear-ops/items/${item.id}/assignments/${assignment.id}/edit/update`}
        method="post"
      ...
      >
        ...
        <FormActions submitLabel="Save assignment" cancelHref={`/gear-ops/items/${item.id}`} />
      </form>
    </section>
  );
}
