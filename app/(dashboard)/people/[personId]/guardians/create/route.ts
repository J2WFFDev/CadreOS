...
function buildErrorRedirectUrl(
  ...
  const url = new URL(`/people/${personId}/guardians/new`, requestUrl);
...
export async function POST(
  ...
  if (!scope.databaseReady) {
    return NextResponse.redirect(
      buildErrorRedirectUrl(request.url, personId, {
        values,
        error: scope.errorMessage ?? "Unable to create guardian relationship right now.",
      }),
      303,
    );
  }
  ...
  try {
    ...
    await requirePhase1CMutationPermission({
      organizationId: organizationId,
      action: "guardianRelationship.create",
    ...
    const existingPair = await db.athleteGuardianRelationship.findFirst({
      where: {
        organizationId: organizationId,
    ...
    if (existingPair) {
      return NextResponse.redirect(
        buildErrorRedirectUrl(request.url, personId, {
          ...
          error:
            existingPair.relationshipType === parsed.data.relationshipType
              ? "Duplicate guardian relationship is not allowed."
              : "This guardian is already linked with a different relationship type. Edit the existing relationship instead.",
        }),
        303,
      );
    }

    const relationship = await db.athleteGuardianRelationship.create({
      data: {
        organizationId: organizationId,
        athletePersonId: person.id,
        guardianPersonId: guardian.id,
    ...
    successUrl.searchParams.set("guardianSuccess", "Guardian relationship created.");
      ...
      buildErrorRedirectUrl(request.url, personId, {
        ...
        error: isPermissionDeniedError(error)
          ...
          : isSchemaUnavailableError(error)
            ? "Database schema is not available yet. Run database setup before creating guardian relationships."
            : "Unable to create guardian relationship right now. Please try again.",
      }),
      303,
    );
  }
