...
export default async function TeamDetailsPage({
  ...
  try {
    ...
    const selectedSeasonId = activeRosterMembership?.season.id ?? demoRosterMembership?.season.id ?? team.roster[0]?.season.id;
    const selectedSeasonName = team.roster.find((membership) => membership.season.id === selectedSeasonId)?.season.name;
    const selectedSeasonRosterMembers = team.roster.filter(
